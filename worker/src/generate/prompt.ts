...
Fill on EVERY beat: shotSize (ECU|CU|MS|WS), cameraAngle (eye|low|high|overhead|pov), durationSec (number, seconds), cutType (hard|match|whip|jump — how this clip ENTERS the edit), startsOnCut, motionBeat (the appeal-carrying body motion this clip must contain — the thing a viewer's eye locks onto), secondaryMotion (hair/fabric/softBody/accessories — what must move BESIDES the action; a beat where all four are "none" is almost always wrong), microExpression (the involuntary life: blink, gaze dart, breath, weight shift), sourceBeatIndex (reproduce: the DNA beat it reproduces; adapt: the DNA beat that inspired it, or -1), and firstFrameSource:
