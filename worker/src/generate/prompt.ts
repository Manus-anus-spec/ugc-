 ...
 */
import type { FidelityMode, ModelProfile, VariationStrength, VideoModelTarget } from '../../../shared/contract';
...
Fill on EVERY beat: shotSize (ECU|CU|MS|WS), cameraAngle (eye|low|high|overhead|pov), durationSec (number, seconds), cutType (hard|match|whip|jump — how this clip ENTERS the edit), startsOnCut, motionBeat (the appeal-carrying body motion this clip must contain — the thing a viewer's eye locks onto), secondaryMotion (hair/fabric/softBody/accessories — what moves BESIDES the action. Fill the fields from observation, but the motionPrompt itself carries only the 1-2 MOST NATURAL cues, e.g. hair moving with a head turn, fabric responding to a weight shift — stacking secondary-motion descriptions makes video models animate the clothing instead of the person; let the rest emerge), microExpression (the involuntary life: blink, gaze dart, breath, weight shift), sourceBeatIndex (reproduce: the DNA beat it reproduces; adapt: the DNA beat that inspired it, or -1), and firstFrameSource:
...
Fill continuityLock ONCE per ideation: setDescription (ONE concrete set), wardrobeExact (exact garments incl. colors), wardrobeKey (the looks.wardrobeDefaults key the outfit came from — lets the pipeline attach the garment reference image), hairExact, lightingExact (sources + direction + behavior), colorTempK (categorical — inherit the DNA aesthetic's when reproducing), timeOfDay, keyProps. EVERY beat's nbPrompt and motionPrompt must stay consistent with this lock — clips are generated independently, and any drift between them (different counter, different top, different light) reads instantly as AI. Continuity is SET + WARDROBE + LIGHT held constant, described through RELATIVE layout (camera height, distance, subject position, headroom, landmarks) — never demand "identical composition" or "exact framing".

# HUMAN PERFORMANCE LAWS (every motionPrompt — this is what separates real phone footage from AI slop)
Write motion prompts the way one creator briefs another before filming — natural narrative sentences, never a technical shot list. These laws are non-negotiable:
1. REACTION, NOT PRESCRIPTION. Never name the expression ("intense pleasure", "confident smirk", "eyes rolling back", "shocked"). Describe what she is REACTING TO and let the expression emerge ("she catches the camera looking at her", "the broth is unexpectedly good"). Expressions DEVELOP across a beat — never instant-swap between two named faces.
2. NO FROZEN LANGUAGE — video models take it literally and render dead frames: never write "pauses", "stands frozen", "freezes", "holds the expression/pose", "briefly holds", "staring", "maintains eye contact", "holds still". Use continuous forms instead: "briefly looks toward…", "her expression shifts as…", "continues moving while…", "glances toward… then naturally returns her attention…". She keeps moving even when the reaction itself is subtle — but do NOT over-correct into constant busy motion that erases the micro-reactions the joke depends on.
3. MICRO-REACTION STRUCTURE for skits/interactions: normal action → notices → visibly reacts → notices being noticed → tiny reaction → plays it off. Bystanders never "stare, shocked"; they run the believable sequence: briefly notices → second glance → realizes he's been noticed → looks away → keeps walking.
4. HUMAN INTERACTION PHYSICS: a half pace closer, never a lunge; a prop lightly taps, never thrust; the camera reacts with a tiny bump like a real phone would; the smile comes before AND after a tease; small behaviors (brushing hair back, a weight shift, a blink, a short laugh) break the AI-pose look.
5. HANDS NEED A REASON, NOT A SCRIPT. Give her a motivation to move her hands (adjusting the prop, brushing her jacket, a hand settling near her hip) — never choreograph constant gestures. Real people move, stop, and settle.
6. CONTINUOUS FLOW. Beats flow INTO each other: each starts from the previous beat's position ("still chewing from the previous bite") — no posture resets, no per-timestamp re-staging. Backgrounds are alive but secondary (a distant couple talking, a server passing once, faint utensil sounds); background people have independent subtle movements and NEVER look at the subject or perform for the camera.
7. FRAMING WITH NUMBERS (environmental third-person/propped MS-WS beats ONLY — never selfie/CU formats): "waist-up", "friend-taken", "front angle" produce PORTRAITS. Write instead: "NOT a portrait, NOT close-up, environmental composition, medium-wide, subject occupies ~35-45% of the frame, room clearly visible around her." Describe RELATIVE layout — camera height, distance, subject position in frame, headroom, background landmarks — never "exact framing" or "identical composition" (diffusion models reproduce spatial relationships reliably; abstract exactness they ignore).
8. THE CAMERA IS A PERSON. A moving third-person camera is someone physically walking: spot her → walk toward her → slightly shaky framing → catch up → slow down → settle into conversational distance. Height at chest/shoulder with a slight 5-10° downward tilt — NOT dead eye-level; never "rapidly approach" (reads as a cinematic push-in). Pans are real phone behavior: a quick handheld pivot, uneven swing, slight overshoot then correction, settles naturally, the operator reacting instinctively — with "strong natural motion blur during the fastest part of the pan", never "rapid whip pan" or "heavy motion blur".
...
1. nbPrompt — NanoBanana still-frame master prompt. Follow toolRules.nb.structureNotes EXACTLY (the 12-step order). Start with identityLock.opener verbatim; end with identityLock.closer verbatim. Include the mandatory makeup block, hair per context, nails, ONE expression from the allowed menu, exact-source lighting, 1-2 imperfect props. Never use toolRules.nb.bannedPhrases. Face clearly visible and facing camera in the FIRST beat of every ideation.
   ...
   ENVIRONMENTAL FRAMING (third-person/propped MS-WS formats only — selfie/mirror/CU formats keep their native framing): NB defaults to centered portraits. Describe the FRAME, not the subject: place her off-center explicitly ("she stands small near the doorway at the right of the frame") and fill the negative space by describing what occupies it ("empty wall and a coat rack fill the left half") — placement + environment-fill controls composition far more reliably than percentages. Add "NOT centered, environmental composition, full body visible with feet in frame, lots of negative space, casual composition as if not intentionally framed", subject around a third of the frame, and describe the layout RELATIVELY (camera height, distance, headroom, background landmarks).
...
3. motionPrompt — video model prompt (Seedance 2.0 primary; Kling fallback). THIS FIELD MUST BE 100% SELF-CONTAINED: the operator copies ONLY this one box into the video model — they will NOT read the action/expression/dialogue side fields, so anything that lives only there is LOST. Compose in this order:
   ...
   + [camera physics — the DNA's camera.dynamics.motionSignature, adapted to this beat's setting: shake, sway, bob, reframes. Use toolRules.video.cameraLines[camera.setup] as the physics BASIS but NEVER paste the same camera sentence verbatim into every beat — the repeated line is an AI tell. Weave the camera into the action in different words each beat, per HUMAN PERFORMANCE LAW 8]
   + [where + outfit]
   + [ONE primary action verb chain, timed to the beat — flowing FROM the previous beat's end position (LAW 6): open with the carry-over ("still mid-laugh from the turn, she…")]
   + [what she's reacting to — the expression emerges and develops from it (LAW 1); never a named expression label]
   ...
   + [environment motion detail — alive but secondary].
   MULTI_CLIP: 350-550 chars per clip WITHOUT dialogue (~60-80 words — short structured beats long poetic); up to 750 chars when the beat carries dialogue (the line + delivery must fit — never truncate the quote).
   ONE_SHOT: a labeled continuous-take choreography sheet, up to 2200 chars, in EXACTLY this block order (the aesthetic anchor still opens the prompt, before the first label):
   CONTINUITY / PERFORMANCE: one continuous take, no cuts, no jump cuts — one candid recording of the SAME subject in the SAME set: same face, hairstyle, outfit, body, props, lighting and camera position throughout; the action flows continuously from one beat into the next with NO freezing, posing, sudden resets or teleporting; every movement begins from the previous movement; realistic human timing. (Seedance WILL cut unless the no-cuts line is explicit.)
   [the timestamped beats — "0.0-0.8s — …" — each flowing from the previous, reaction-driven, per the LAWS above]
   ENVIRONMENT: the set alive but secondary — background people with small independent movements who never look at the subject; understated realistic ambience.
   CAMERA: who holds the phone, at what height and distance, how it reacts to her — and the explicit blocks: no artificial camera movements, no dramatic zooms, no stabilization, no slow motion, no frame interpolation.
   PERFORMANCE: natural idle human behavior (blinking, small eye saccades, breathing in the shoulders, subtle weight shifts, natural finger relaxation) + physical object consistency (props/food stay physically consistent, gravity respected).
   AUDIO / DIALOGUE: room tone per the DNA; dialogue with delivery per the DIALOGUE DELIVERY rules; no subtitles, no captions, no watermark.
...
- cdance_2 (Seedance 2.0 — the PRIMARY production model): honors the EARLIEST strong instruction — aesthetic anchor stays first. Natural human narrative phrasing beats technical directives ("she keeps stirring while glancing at the camera" > "subject maintains engagement, medium shot"). ONE camera behavior per beat (stacking pan+push+move destabilizes the image). Keep camera-motion and subject-action in SEPARATE sentences (never "camera spins around her dancing"). Dialogue goes in curly braces with delivery outside: she says casually {exact line} — keep lines ~5-10 words (over ~12 words per 10s clip the voice rushes unnaturally; split long lines with a written beat: "she pauses, then continues:"), and add "voice sounds like a phone microphone, natural room tone, no music". Seedance generates the speech audio natively — specify voice character (age/accent/energy) in the delivery or it drifts between clips. For handheld mess the negation pair is load-bearing: include "no smoothness, no stabilization" or it drifts gimbal-smooth. Rhythm words (slow, gentle, natural) beat technical specs; never apply "fast" to more than one element. Timestamps inside a take are ORDERING hints, not precise timing (officially unstable) — the model distributes the beats; the edit's trim map recovers exact cadence. A camera behavior sequence is two beats in separate sentences with explicit deceleration ("the camera walks toward her; then it slows and settles at conversational distance"). ALWAYS end the motionPrompt with: "keep it subtitle-free, avoid generating any text or subtitles, no watermark".
- kling_3 (fallback model — only when the run targets Kling): defaults smooth/cinematic — push it toward mess EXPLICITLY with "handheld phone footage, handheld micro-shake, slight autofocus breathing, flat indoor lighting (not cinematic), phone camera look not commercial, minor framing imperfections". For Kling put the camera instruction LAST in the prompt (it weights trailing camera language best).
...
- videoModel: ${videoModelTarget === 'kling'
    ? 'this run targets KLING — kling_3 for single-scene no-dialogue content; cdance_2 ONLY when the ideation genuinely needs dialogue/lip-sync, multi-clip transitions, or dramatic expression work. Write the honest reason.'
    : 'cdance_2 (Seedance 2.0) for EVERY ideation — it is the production model this pipeline targets. Write the honest reason it fits this treatment.'}
