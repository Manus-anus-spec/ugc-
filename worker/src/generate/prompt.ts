...
A beat is one video-generation unit: one first frame (GPT-Image-2 one-shot; Seedream body pass only if needed) → ONE motion prompt → one generated clip.
...
# ONE-SHOT LAW (default clip structure)
- Total runtime ≤15s in ONE continuous scene → videoFormat ONE_SHOT: one take, one choreography-sheet motionPrompt in phase-flow ("First … then … finally …", each phase flowing out of the previous — she is still mid-motion from the last phase). NEVER chop a short idea into separate clips to imitate cut cadence — cuts are recreated in the EDIT by slicing the single take.
- Use MULTI_CLIP ONLY when (a) a hard scene change makes one take physically impossible (bedroom→kitchen; skits/dialogue across distinct scenes), or (b) total runtime >15s. When splitting: the FEWEST, LONGEST takes possible — never plan a clip under 5s, never 3s slivers.
...
1. nbPrompt — NanoBanana still-frame master prompt. Follow toolRules.nb.structureNotes EXACTLY (the 12-step order). Start with identityLock.opener verbatim; end with identityLock.closer verbatim. Include the mandatory makeup block, hair per context, nails, ONE expression from the allowed menu, exact-source lighting, 1-2 imperfect props. Never use toolRules.nb.bannedPhrases. Face clearly visible and facing camera in the FIRST beat of every ideation.
   ...
   CANDID, NOT POSED (§1): default the gaze OFF-camera or mid-action with a genuine in-the-moment expression — never "staring into the lens", never a blank model stare. Camera pulled back (subject ~35-45% of frame), sharp cluttered REAL background in deep focus (NO bokeh). Lighting: flat harsh natural daylight with hard shadows by default — never golden hour, warm glow, backlit, soft light, or rim light (those read "produced photoshoot").
   PROP-PHYSICS LAW (§4): any prop that emits, pours, sprays, or burns (hose, bottle, spray, smoke, tap) must have its SOURCE established — name the visible source in the first-frame prompt (visible hose NOZZLE, open bottle mouth, cut face of the fruit) AND make it already-active at clip start in the motion prompt (water ALREADY running when the clip opens). Never let an effect appear without its physical cause in frame; never spawn a tool (knife) that was not established.
...
3. motionPrompt — video model prompt. THIS FIELD MUST BE 100% SELF-CONTAINED: the operator copies ONLY this one box into Kling/CDance — they will NOT read the action/expression/dialogue side fields, so anything that lives only there is LOST. Compose in this order:
   ...
   MULTI_CLIP: 350-550 chars per clip WITHOUT dialogue (~60-80 words — short structured beats long poetic); up to 750 chars when the beat carries dialogue (the line + delivery must fit — never truncate the quote). ONE_SHOT: a STRUCTURED choreography sheet up to 1200 chars — this is now the MAIN output, build it in this order: (1) LOOK declaration FIRST, defending the first frame ("keep the raw phone look of the first frame" — do NOT re-describe the scene; the first frame owns the look), (2) the action PHASES in continuous flow ("First … then … finally …"), each phase flowing out of the previous, ONE camera behavior per phase with camera-motion and subject-action in SEPARATE sentences, (3) audio (location-true ambient + phone mic), (4) constraints LAST (subtitle/watermark ban). Base the camera on toolRules.video.cameraLines matching the DNA camera.setup, but INTEGRATE it into the flow at chest/shoulder height — do NOT repeat the same camera sentence verbatim on every beat (that identical-line repetition is an AI tell); and default to a STATIC locked-off phone (§10) unless the source genuinely moved. Never use toolRules.video.bannedWords; "slowly" at most once; subtle/gentle/soft at most twice total. Study toolRules.video.confirmedWorkingExamples as the house style — for ONE_SHOT choreography sheets they are the ANCHOR: match their voice, rhythm, and level of concreteness (validated live keepers, not theory).
